...
def liquidity(request):
    ...
    liquidity_array = Liquidity.objects.values_list("type_id", "day_volume", "day_turnover", "price", "price_sell",
                                                    "price_bay"). \
        filter(region_id=int(region_id)).order_by("-day_turnover")[:config.counts_on_page]
    liquidity_to_page = []
...
def planing(request):
    ...
    if region_id_from < region_id_to:
        logistics_planing_array = LogisticsPlanning.objects.values_list(
            ...
            .filter(region_id_from=int(region_id_from), region_id_to=int(region_id_to)).order_by("-profit_from")[:config.counts_on_page]
        ...
        logistics_planing_array = LogisticsPlanning.objects.values_list(
            ...
            .filter(region_id_from=int(region_id_to), region_id_to=int(region_id_from)).order_by("-profit_to")[:config.counts_on_page]
        for log in logistics_planing_array:
            name = Types.objects.values_list("name").filter(type_id=log[0])
