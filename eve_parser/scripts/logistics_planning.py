...
def run(*args):
    ...
    for reg_from in region_from:
        for reg_to in region_to:
            ...
            Parser.parser_date_status("Calculate logistic", reg_from[0], reg_to[0])
...
def check_need(region_from, region_to):
    ...
    liq_to = ParserDateStatus.objects.get(parser_name="Liquidity calculation", region_id=int(region_to))
